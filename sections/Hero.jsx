const heroStyles = {
  section: { position: 'relative', background: 'var(--canvas)', overflow: 'hidden', padding: '156px 24px 32px' },
  ...
  h1: { fontFamily: 'var(--font-display)', fontWeight: 500, fontSize: 'var(--text-display-xl)', lineHeight: 'var(--lh-tight)', letterSpacing: '-0.035em', color: 'var(--ink)', margin: 0, textWrap: 'balance' },
  sub: { fontFamily: 'var(--font-body)', fontSize: 18, lineHeight: 1.6, color: 'var(--ink-2)', margin: '0 0 28px' },
  ctas: { display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' },
  risk: { fontFamily: 'var(--font-body)', fontSize: 14, color: 'var(--ink-3)', margin: '18px 0 0' },
  stage: { position: 'relative', background: 'var(--night)', borderRadius: 'var(--radius-xl)', overflow: 'hidden', padding: '64px 40px 28px', boxShadow: 'var(--shadow-glow-accent)' },
  stageArt: { position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center right', filter: 'grayscale(1) contrast(1.1)', opacity: 0.22, mixBlendMode: 'screen', pointerEvents: 'none' },
  stageFade: { position: 'absolute', inset: 0, background: 'linear-gradient(90deg, var(--night) 0%, rgba(42,43,34,.6) 45%, rgba(42,43,34,.15) 100%)', pointerEvents: 'none' },
  tag: { position: 'absolute', top: -38, transform: 'translateX(-50%)', fontFamily: 'var(--font-mono)', fontWeight: 500, fontSize: 12, color: 'var(--on-night-2)', border: '1px solid var(--hairline-night)', borderRadius: 'var(--radius-pill)', padding: '4px 10px', whiteSpace: 'nowrap' },
  resolvedTag: { position: 'absolute', right: 0, top: -40, fontFamily: 'var(--font-accent)', fontSize: 11, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--night)', background: 'var(--lime)', borderRadius: 'var(--radius-pill)', padding: '6px 12px', whiteSpace: 'nowrap' },
  caption: { position: 'relative', display: 'flex', justifyContent: 'space-between', gap: 16, fontFamily: 'var(--font-mono)', fontWeight: 600, fontSize: 12, color: 'var(--on-night-3)', letterSpacing: 'var(--tracking-wide)', textTransform: 'uppercase', padding: '20px 0 0', borderTop: '1px solid var(--hairline-night)', marginTop: 18 },
};
const CHAOS_TAGS = [
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="auto" preserveAspectRatio="none" style={{ display: 'block', overflow: 'visible' }} aria-hidden="true">
        <defs>
          <linearGradient id="heroPulseGrad" x1="0" y1="0" x2="1" y2="0">
            <stop offset="0%" stopColor="#F4F6EC" stopOpacity="0.28" />
            <stop offset="44%" stopColor="#F4F6EC" stopOpacity="0.5" />
            <stop offset="66%" stopColor="#D4ED57" />
            <stop offset="100%" stopColor="#D4ED57" />
          </linearGradient>
        </defs>
    ...
    <section id="topo" style={heroStyles.section} data-screen-label="Hero">
      ...
      <div style={heroStyles.inner}>
        <div className="mb-split" style={heroStyles.top}>
          ...
          <div>
            ...
            <div className="mb-rise mb-rise-3" style={heroStyles.ctas}>
              ...
              <Button variant="ghost" size="lg" onClick={go('solucoes')}>Conheça nossas soluções</Button>
            </div>
        ...
        <div className="mb-rise mb-rise-4 mb-stage" style={heroStyles.stage}>
          <img className="mb-hide-sm" src="./images/mambolabs-hero.png" alt="" aria-hidden="true" style={heroStyles.stageArt} />
